...
export function resolveOrganizationName(siteName: string | undefined | null): string {
...
/** Original certificate layout (classic bordered design with gold seal). */
...
<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1120" viewBox="0 0 1600 1120">
  ...
  <text x="800" y="622" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="27" fill="#334155">${safePhrase}</text>
  ${courseLines
    .map(
      (line, index) =>
  ...
  <g font-family="Arial, Helvetica, sans-serif" fill="#334155">
    ${orgFooterLines
      .map((line, index) => {
        const lineCount = orgFooterLines.length;
        const orgY = lineCount === 1 ? 838 : 818 + index * 28;
        return `<text x="310" y="${orgY}" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="22" font-weight="700" fill="#0f766e">${escapeXml(line)}</text>`;
      })
      .join("")}
    <line x1="205" y1="862" x2="415" y2="862" stroke="#94a3b8" stroke-width="2"/>
  ...
  <text x="800" y="1008" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="16" fill="#94a3b8">Certificate ID: ${safeCertificateId}</text>
</svg>`;
}
