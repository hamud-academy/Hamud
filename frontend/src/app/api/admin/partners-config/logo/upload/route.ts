...
export async function POST(request: NextRequest) {
  ...
  if (!file || !(file instanceof File)) {
    return NextResponse.json({ error: "No file. Please select a logo image." }, { status: 400 });
  }
  ...
  const filename = `partner-logo-${Date.now()}${safeExt}`;
  const bytes = await file.arrayBuffer();
  const buffer = Buffer.from(bytes);
  const relativePath = `uploads/partners/${filename}`;
  ...
  try {
    ...
    console.error("Partner logo upload error:", e);
